...
class NaturalLanguageQASystem:
    ...
    def _search_docs(
        ...
        try:
            ...
            with timed_block("Semantic document search"):
    ...
    def _get_candidate_docs(
        ...
        for result in results:
            ...
            #     doc["answer_highlights"] = [
            #         ans.text for ans in result["@search.answers"]
            #     ]
            docs.append(doc)
    ...
    #     #     snippet = doc.get("merged_content", "").replace("\n", " ")
    #     #     score = doc.get("score", 0)
    #     #     products_context.append(f"Product {i} (score: {score:.2f}): {snippet}")
    #     products_context = []
    #     for i, doc in enumerate(limited_docs, 1):
    ...
    #             f"Size: {doc.get('Size', 'N/A')} | "
    #             f"Type: {doc.get('Type', 'N/A')} | "
    #             f"PCS/BU: {doc.get('PCSBU', 'N/A')} | "
    #             f"PCS/SKU: {doc.get('PCSSKU', 'N/A')} | "
    #             f"SKU/BU: {doc.get('SKUBU', 'N/A')} | "
    #             f"Image URL: {doc.get('image_url', 'N/A')} | "
    #             f"Score: {doc.get('score', 0):.2f}"
    #         )
    #     products_text = "\n".join(products_context) or "No products found."

    #     return f"""
    #         You are a product advisor. Based on the user's question and the relevant product descriptions, generate a concise structured JSON response.
    #         ## User Question
    #         {question}

    #         ## Relevant Products Descriptions
    #         {products_text}
    ...
    #             f"Size: {best_match.get('Size', 'N/A')}, "
